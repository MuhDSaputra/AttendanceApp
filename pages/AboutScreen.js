...
export default function AboutScreen() {
  ...
  const NIM_USER = "0320240089"; // Simulasi NIM Mahasiswa
  const BASE_URL = "http://10.1.13.46:8080/api/mahasiswa";
  ...
  const uploadPhoto = async (uri) => {
    ...
    formData.append("nama", "Muhammad Dwi Saputra"); // Contoh nama
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert("Sukses", responseText);
        fetchMahasiswa();
      } else {
    ...
    <View style={styles.container}>
      <View style={styles.profileCard}>
        ...
        <Text style={styles.nameText}>
          {mahasiswa?.namaMhs || "Muhammad Dwi Saputra"}
        </Text>
